...
nemert <- read.csv("nemertea_taxonomy.csv", header=T, as.is=F)
...
oto_33 <- droplevels(oto_m$species)

DNA_dist_oto1 <- DNA_dist[rownames(DNA_dist) %in% oto_33,]
DNA_dist_oto2 <- t(DNA_dist_oto1)
DNA_dist_oto <- DNA_dist_oto2[rownames(DNA_dist_oto2) %in% oto_33,]
